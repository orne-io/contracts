...
use orne_token::common::OrderBy;
use orne_token::vesting::{
...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            orne_token: deps.api.addr_canonicalize(&msg.orne_token)?,
        ...
        _ => {
            ...
            match msg {
                ExecuteMsg::UpdateConfig {
                    ...
                    orne_token,
                    genesis_time,
                } => update_config(deps, owner, orne_token, genesis_time),
...
pub fn update_config(
    ...
    if let Some(orne_token) = orne_token {
        config.orne_token = deps.api.addr_canonicalize(&orne_token)?;
    }
...
pub fn claim(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let messages: Vec<CosmosMsg> = if claim_amount.is_zero() {
        ...
        vec![CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.orne_token)?.to_string(),
            funds: vec![],
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        ...
        orne_token: deps.api.addr_humanize(&state.orne_token)?.to_string(),
